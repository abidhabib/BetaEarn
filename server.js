...
app.post('/updateBalance', (req, res) => {
    ...
    con.query(checkLastClickedSql, [req.session.userId, productId], (err, result) => {
        ...
        if (result.length > 0) {
            ...
            if (timeDifference < 12 * 60 * 60 * 1000) { // Less than 12 hours
                return res.json({ status: 'error', error: 'You have completed your task' });
            }
        }

        // Step 1: Update the user balance
        ...
        con.query(updateBalanceSql, [reward, req.session.userId], (err, updateResult) => {
            ...
            // Step 2: Subtract the reward from today_wallet
            const updateTodayWalletSql = `UPDATE users SET today_wallet = today_wallet - ? WHERE id = ?`;
            con.query(updateTodayWalletSql, [reward, req.session.userId], (err, updateWalletResult) => {
                if (err) {
                    console.log('Error updating today_wallet:', err);
                    return res.status(500).json({ status: 'error', error: 'Failed to update today_wallet' });
                }

                // Step 3: Update the last clicked time for the product
                ...
                con.query(updateLastClickedSql, [req.session.userId, productId, currentTime], (err, clickResult) => {
                    ...
                    return res.json({ status: 'success', message: 'Balance, today_wallet, and last clicked time updated successfully' });
                });
            });
        });
    });
...
app.get('/approved-users', async (req, res) => {
    ...
    let sql = `SELECT id, balance, team, name, email, phoneNumber, backend_wallet, trx_id, total_withdrawal, CurrTeam, refer_by, password, level_updated, level FROM users WHERE approved = 1 AND payment_ok = 1`;
    ...
    con.query(countSql, (countErr, countResult) => {
        ...
        sql += ` ORDER BY ${sortKey} ${sortDirection} LIMIT ?, ?`;

        // Calculate offset for pagination
        const offset = (page - 1) * perPage;

        con.query(sql, [offset, perPage], async (err, result) => {
            if (err) {
            ...
            const usersWithFinalResult = await Promise.all(result.map(async (user) => {
                const userId = user.id;

                // Get user credits, backend_wallet, balance, total_withdrawal, and today_wallet
                ...
                if (userResults && userResults.length > 0) {
                    const { all_credits, backend_wallet, balance, total_withdrawal, today_wallet } = userResults[0];

                    // Get the sum of the amounts from the bonus_history table
                    ...
                    // Get the sum of the amounts from the bonus_history_level_up table
                    ...
                    // Attach the final result to the user data
                    return { ...user, finalResult };
                }
                return { ...user, finalResult: 0 };
            }));
...
app.get('/get-accounts', (req, res) => {
    const sql = 'SELECT * FROM accounts   limit 5';
...
app.put('/updateUser', (req, res) => {
    ...
    const { id, name, email, balance, CurrTeam, trx_id, total_withdrawal, level, level_updated, backend_wallet } = req.body;
    ...
    con.query(sql, [name, email, balance, CurrTeam, trx_id, total_withdrawal, level, level_updated, backend_wallet, id], (err, result) => {
        if (err) {
            console.error(err);
...
app.post('/trackButton', (req, res) => {
    ...
    const updateBalanceSql = `
      UPDATE users
      SET balance = balance + ?,all_credits=all_credits+?
      WHERE id = ?;
    `;
...
app.get('/withdrawal-requests', (req, res) => {
    ...
    const sql = 'SELECT user_id, request_date, reject,account_number,fee, amount, bank_name,account_name, approved FROM withdrawal_requests WHERE user_id = ? ORDER BY request_date DESC';
    ...
    try {
        ...
        if (referrerId) {
            ...
            await queryAsync(`
                ...
                WHERE id = ?
            `, [directBonusAmount, indirectBonusAmount, extraBalanceAmount, directBonusAmount, indirectBonusAmount, referrerId]);
...
app.get('/fetchClickedProducts', (req, res) => {
    ...
    const userId = req.session.userId;
    const today = new Date().toISOString().split('T')[0];  // Get the date (yyyy-mm-dd)

    const getClickedProductsSql = `
        SELECT p.*, upc.last_clicked
        FROM products p
        LEFT JOIN user_product_clicks upc ON p.id = upc.product_id AND upc.user_id = ?
    `;

    con.query(getClickedProductsSql, [userId], (err, productResults) => {
        if (err) {
            console.error('Error fetching clicked products:', err);
        ...
        if (productCount > 0) {
            const updateWalletSql = `
                ...
                SET 
                    today_wallet = CASE
                        WHEN COALESCE(last_wallet_update, '') <> ? THEN today_wallet+backend_wallet * 0.1
                        ELSE today_wallet
                    END,
                    backend_wallet = CASE
            ...
            con.query(updateWalletSql, [today, today, today, today, userId, today], (err) => {
                if (err) {
                ...
                con.query(getUserDataSql, [userId], (err, userResults) => {
                    if (err) {
                        console.error('Error fetching user wallet data:', err);
                        return res.status(500).json({ status: 'error', error: 'Failed to fetch user wallet data' });
                    }
...
app.get('/approved-users-spec', (req, res) => {
    const page = parseInt(req.query.page) || 1;
 


    let sql = `SELECT id,balance,team,name,email,phoneNumber,backend_wallet,trx_id,total_withdrawal,CurrTeam,refer_by,password FROM  users
    WHERE 
        approved = 1
        AND payment_ok = 1 AND id  between 9329 and 9338`;
...
const fetchApprovedUserNames = (referByUserId) => {
    return new Promise((resolve, reject) => {
        const fetchNamesQuery = 'SELECT id, name ,team, approved_at FROM users WHERE refer_by = ? AND approved = 1';
